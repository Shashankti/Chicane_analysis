...
Out of these significant interactions MALAT1 and NEAT1 account for `r length(ibed_signif_stretch[ibed_signif_stretch$bait_name=="MALAT1"|ibed_signif_stretch$bait_name=="NEAT1"]$target.id)/length(ibed_signif_stretch$distance)*100` 
Filtering this data for the significant interactions.
...
p1 = ggplot(ibed_signif_stretch,aes(distance))+
  geom_density(fill="#69b3a2", color="#e9ecef",alpha=0.8)+
  ggtitle("Distribution of distance for radicl-seq(q.value)")+
  theme_cowplot()+
  theme(plot.title = element_text(size = 10))
p2 = ggplot(chic_signif[chic_signif$score>5],aes(distance))+
  geom_density(fill="#69b3a2", color="#e9ecef",alpha=0.8)+
  ggtitle("Distribution of distance for radicl-seq(score)")+
  theme_cowplot()+
  theme(plot.title = element_text(size = 10))
p1 = xmlSVG({show(p1)}, standalone = TRUE)  
p2 = xmlSVG({show(p2)}, standalone = TRUE)
...
```{r}
p1 = ggplot(without_ibed[without_ibed$q.value<0.05],aes(distance))+
  geom_density(fill="#69b3a2", color="#e9ecef",alpha=0.8)+
  ggtitle("Distribution of distance without MALAT1 and NEAT1")+
  theme_cowplot()+
  theme(plot.title = element_text(size = 10))
p2 = ggplot(wo_mn[wo_mn$score>5],aes(distance))+
  geom_density(fill="#69b3a2", color="#e9ecef",alpha=0.8)+
  ggtitle("Distribution of distance for radicl-seq(score)")+
  theme_cowplot()+
  theme(plot.title = element_text(size = 10))
...
```{r}
p1 = ggplot(signif[signif$score<5 & is.na(signif$distance)] %>% arrange(distance),aes(x=log10(q.value),y=log(score),base=10))+
  ...
  theme(plot.title = element_text(size = 10))
p2 = ggplot(signif[signif$score<5 & !is.na(signif$distance)] %>% arrange(distance),aes(x=log10(q.value),y=log(score),base=10))+
  geom_point(aes(color=distance),size=0.5,alpha=0.3)+
    scale_colour_viridis()+
    xlab("log10 q.value")+
  ylab("log10 score")+
